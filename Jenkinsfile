node {
    ...
    stage('Build image') {
        app = docker.build(" apiapp/python-test", "-f ./server/Dockerfile ./server")
    }

    stage('Test image') {
        sh "docker run  -v ${hostReportsAbsPath}:/tests apiapp/python-test pytest -q --junitxml=/tests/report.xml"
    }
    ...
    stage('Deploy') {
        ...
        sh "docker run -d -p 8081:8080 --name python-test apiapp/python-test:${versionTag}"
    }
